...
function t(key, replacements = {}) {
    let text = fractionTranslations[currentLang]?.[key] || fractionTranslations.ru[key];
...
function displayResult(numerator, denominator) {
    ...
    if (denominator === 1) {
        resultDiv.innerHTML = `
            <div style="font-size: 28px; color: #00ffff; text-shadow: 0 0 10px #00ffff; font-weight: bold;">
                ${numerator}
            </div>
        `;
    } else {
        resultDiv.innerHTML = `
            <div style="text-align: center;">
                <div style="font-size: 24px; color: #00ffff; font-weight: bold;">${numerator}</div>
                <div class="fraction-line" style="margin: 8px auto; width: 60px; height: 2px; background: #00ffff;"></div>
                <div style="font-size: 24px; color: #00ffff; font-weight: bold;">${denominator}</div>
            </div>
        `;
    }
}
...
function showError(message) {
    resultDiv.innerHTML = `<div class="error" style="color: #ff0066; text-shadow: 0 0 10px #ff0066;">${message}</div>`;
}
